...
const SignupForm: React.FC = () => {
  ...
  const [country, setCountry] = useState<keyof typeof countryListAlpha2 | ''>('');
  ...
  const mutation = useMutation<unknown, Error, Partial<Participant>>({
    mutationFn: async ()=> {
      const res = await addParticipant({ name, country } as Participant);
  ...
  const handleSubmit = (e: FormEvent) => {
    e.preventDefault();
    mutation.mutate({ name, country } as Partial<Participant>);
  };
      ...
      <p>I hereby confirm that I will do my best to be as friendly as I possibly can towards all human beings I meet, showing them that I wish them well—even those I don’t like or disagree with.</p>
        ...
        <form action="#" method="POST" className="mt-4 max-w-sm mx-auto"  onSubmit={handleSubmit}>
          ...
          <div className="mb-4">
              ...
              <select id="country" onChange={(e) => setCountry(e.target.value as keyof typeof countryListAlpha2)} name="country" className="w-full px-4 py-2 border rounded-lg focus:outline-none focus:border-blue-500" defaultValue={''} required>
        ...
        {(participants ?? []).map((participant, index) => (
          <div className="flex items-center space-x-2" key={index}>
            ...
            <span className="normal-case">{(countryListAlpha2[participant.country]) as any ?? participant.country}</span>
