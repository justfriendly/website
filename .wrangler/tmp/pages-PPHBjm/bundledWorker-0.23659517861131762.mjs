...
var ie = (e, t, a, s) => {
  if (t && typeof t == "object" || typeof t == "function")
    for (let n of ae(t))
      !ne.call(e, n) && n !== a && V(e, n, { get: () => t[n], enumerable: !(s = se(t, n)) || s.enumerable });
  return e;
};
var j = (e, t, a) => (a = e != null ? te(re(e)) : {}, ie(t || !e || !e.__esModule ? V(a, "default", { value: e, enumerable: true }) : a, e));
var g;
var l = N(() => {
  g = { collectedLocales: [] };
});
...
async function K(e) {
  ...
  try {
    ...
    switch (e.method) {
      case "GET": {
        let i = G(e, fe), r = await s.get(n, { softTags: i });
      ...
      case "POST": {
        let i = await e.json();
        return i.data.tags === void 0 && (i.tags ??= G(e, pe) ?? []), await s.set(n, i), new Response(null, { status: 200 });
      }
      default:
...
var T = class {
  ...
  applyLocaleRedirects(t) {
    if (!t.locale?.redirect || !/^\^(.)*$/.test(t.src) && t.src !== this.path || this.headers.normal.has("location"))
      return;
    let { locale: { redirect: s, cookie: n } } = t, i = n && this.cookies[n], r = H(i ?? ""), o = H(this.reqCtx.request.headers.get("accept-language") ?? ""), p = [...r, ...o].map((y) => s[y]).filter(Boolean)[0];
    if (p) {
  ...
  async checkPhase(t) {
    ...
    let n = "miss";
    return s || t === "miss" || t === "error" ? n = "hit" : a && (n = I(t)), this.checkPhase(n);
  }
  async run(t = "none") {
 ...
 * MIT Licensed
 */
//# sourceMappingURL=bundledWorker-0.23659517861131762.mjs.map
